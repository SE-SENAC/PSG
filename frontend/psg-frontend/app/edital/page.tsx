...
export default function Edital(){
    ...
    const [editais,setEditais] = useState<ResultadoInterface[]>([]);
    const headers = ["Título","Criado em","Data de Atualização", "Download"]
    const [current_page,setCurrentPage] = useState<number>(1);
    const [meta,setMeta] = useState<MetaInterface>();

    const handleEdital = async()=>{
        try{
            let response = await EditalService.findAll(current_page);
    ...
    return(
        <div className="container mx-auto space-y-20 pl-10 sm:pl-20 mt-20 mb-10 sm:h-[90vh]">
            ...
            <Table className="h-[50vh]">
                <TableHeader>
                    <TableRow>
                        {headers.map((x,index) => <TableHead key={index} className={` ${x === "Título" ? 'w-[65%]' : ''}`}>{x}</TableHead>)}
                    </TableRow>
                </TableHeader>
                <TableBody>
                    {editais?.length > 0 ? editais?.map((x : any,index) =>
                        <MotionTableRow key={index} variants={motionVariants} initial="hidden" animate="visible" transition={{ duration: 0.3, delay: 0.1 * index }}>
                            ...
                            <TableCell><Link href={`/${x.file_path}`} download={"edital.pdf"}><Button className="bg-[#FF9200] hover:bg-[#FF9200]/90 text-light cursor-pointer"><Download/> Download</Button></Link></TableCell>
                        </MotionTableRow>) 
                        : 
                        <TableRow>
                            <TableCell className="h-24 text-center" colSpan={headers.length}>Não existem editais</TableCell>
                        </TableRow>
                    }
                </TableBody>
            </Table>
