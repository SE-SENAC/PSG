...
export default function UpdateGuidelinePage() {
    ...
    const diretrizId = params.diretrizId as string;
    ...
    useEffect(() => {
        const fetchGuideline = async () => {
            try {
                const res = await DiretrizesService.findById(diretrizId);
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        try {
            ...
            formData.append('title', guideline.title || '');
            
            await DiretrizesService.update(diretrizId, formData);
            toast.success('Diretriz atualizada com sucesso!');
            router.push(`/gerenciador/admin/${adminId}/dashboard/diretrizes`);
        } catch (err: any) {
            console.error(err);
            toast.error('Erro ao atualizar diretriz');
        } finally {
            setSubmitting(false);
        }
    };
    ...
    return (
        <div className="max-w-4xl mx-auto space-y-8 pb-20">
            <div className="flex items-center gap-4">
                ...
                <div>
                    <h1 className="text-3xl font-black text-[#004587] tracking-tight">Editar Diretriz</h1>
                    <p className="text-slate-500 font-medium">{guideline?.title}</p>
                </div>
            </div>
