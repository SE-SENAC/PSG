...
export default function UpdateEditalPage() {
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        try {
            ...
            await EditalService.update(editalId, formData);
            toast.success('Edital atualizado com sucesso!');
            router.push(`/gerenciador/super-admin/${adminId}/dashboard/edital`);
        } catch (err: any) {
            toast.error(err.response?.data?.message || 'Erro ao atualizar edital');
        } finally {
            setSubmitting(false);
        }
    };
    ...
    return (
        <div className="max-w-2xl mx-auto space-y-8 pb-20">
            <div className="flex items-center gap-4">
                ...
                <h1 className="text-3xl font-black text-[#004587] tracking-tight">Atualizar Edital</h1>
            </div>
            ...
            <form onSubmit={handleSubmit} className="space-y-6">
                ...
                <Card className="rounded-[40px] border-none shadow-2xl shadow-slate-200/50 overflow-hidden">
                    ...
                    <CardContent className="p-10 space-y-6">
                        <div className="space-y-3">
                            ...
                            />
                            {edital?.file_path && !file && (
                                <p className="text-xs text-slate-500">Arquivo atual: {edital.file_path.split('/').pop()}</p>
                            )}
                        </div>
                    </CardContent>
                </Card>
                ...
                <div className="flex gap-4 pt-6">
                    ...
                    >
                        ...
                        {submitting ? 'Atualizando...' : 'Atualizar Edital'}
                    </Button>
                    <Button 
                        type="button"
                        onClick={() => router.back()}
