...
import { User, CircleAlert,Lock } from "lucide-react";
...
export default function Login() {
    ...
    const handleLogin = async () => {
        if (verifyForm()) {
            setLoading(true);
            try {
                let auth = await AuthServices.loginAdmin({ email, password });
                dispatch(login({ user: auth.userExists, token: auth.token }));
                router.push(`/auth-manager/admin/${auth.userExists.id}/dashboard/general`);
            } catch (e) {
                throw e;
            }finally{
                setLoading(false);
        ...
        >
            <div className="h-[95vh] flex justify-center items-center">
                <div className="shadow-lg rounded-md p-12 space-y-4 sm:w-1/4 bg-card border border-border">
                    ...
                    <h1 className="text-2xl text-foreground text-center uppercase font-bold">Entrar Administrador</h1>
